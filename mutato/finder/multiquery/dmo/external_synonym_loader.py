...
from mutato.core import configure_logging, Stopwatch, FileIO, isEnabledForDebug

class ExternalSynonymLoader(object):
    """ Load a Supplementary External File of Synonyms """
    ...
    def process(self) -> dict:
        ...
        if isEnabledForDebug(self.logger):
            self.logger.debug(
                f"Loaded External File from ({self._synonym_file_path}) with size={len(d_file)} in {str(sw)}")
        return d_file
